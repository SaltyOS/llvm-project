//===--- SaltyOS.cpp - SaltyOS ToolChain Implementations ------*- C++ -*-===//
//
...
// SaltyOS linker
void saltyos::Linker::ConstructJob(Compilation &C, const JobAction &JA,
                                   const InputInfo &Output,
                                   const InputInfoList &Inputs,
                                   const ArgList &Args,
                                   const char *LinkingOutput) const {
  const auto &TC = static_cast<const SaltyOS &>(getToolChain());
  ...
  if (!Args.hasArg(options::OPT_nostdlib, options::OPT_T) &&
    ...
    CmdArgs.push_back("-T");
    CmdArgs.push_back(Args.MakeArgString(TC.GetFilePath("trona-pie.ld")));
  }
...
// SaltyOS toolchain
SaltyOS::SaltyOS(const Driver &D, const llvm::Triple &Triple,
                  const ArgList &Args)
    : Generic_ELF(D, Triple, Args) {
  getFilePaths().push_back(D.SysRoot + "/usr/lib");
}

Tool *SaltyOS::buildLinker() const {
  return new tools::saltyos::Linker(*this);
}

void SaltyOS::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
